...
namespace Microsoft.EntityFrameworkCore.TestUtilities.QueryTestGeneration
{
    ...
    public class ProcedurallyGeneratedQueryExecutor
    {
        ...
        static ProcedurallyGeneratedQueryExecutor()
        {
            ...
            AddExpectedFailure("GroupBy_Key_as_part_of_element_selector", "Incorrect syntax near the keyword 'AS'."); // 12650
            AddExpectedFailure("GroupJoin_GroupBy_Aggregate_5", "Incorrect syntax near the keyword 'AS'."); // 12650
            AddExpectedFailure("GroupBy_Property_Include_Select_Sum", "Incorrect syntax near the keyword 'AS'."); // 12650
            AddExpectedFailure("GroupBy_Property_Include_Select_Min", "Incorrect syntax near the keyword 'AS'."); // 12650
            AddExpectedFailure("GroupBy_Property_Include_Select_Max", "Incorrect syntax near the keyword 'AS'."); // 12650
            AddExpectedFailure("GroupBy_Property_Include_Select_Count", "Incorrect syntax near the keyword 'AS'."); // 12650
            AddExpectedFailure("GroupBy_Property_Include_Select_Average", "Incorrect syntax near the keyword 'AS'."); // 12650
            AddExpectedFailure("GroupBy_Property_Include_Select_LongCount", "Incorrect syntax near the keyword 'AS'."); // 12650
            ...
            AddExpectedFailure("SelectMany_GroupBy_Aggregate", "Incorrect syntax near the keyword 'AS'."); // 12818
            AddExpectedFailure("Select_anonymous_GroupBy_Aggregate", "Incorrect syntax near the keyword 'AS'."); // 12818
            AddExpectedFailure("Anonymous_projection_Distinct_GroupBy_Aggregate", "Incorrect syntax near the keyword 'AS'."); // 12818
        ...
        public void Execute<TElement>(IQueryable<TElement> query, DbContext context)
        {
            ...
            catch (Exception exception)
            {
                ...
                else if (exception.Message == @"The SELECT item identified by the ORDER BY number 1 contains a variable as part of the expression identifying a column position. Variables are only allowed when ordering by an expression referencing a column name.")
                {
                    // state machine complicates the stack trace - hard to pinpoint the method name
                }
                else
                {
                    ...
                    var assertQueryMethods = new[] { "AssertQuery", "AssertIncludeQuery" };

                    var testMethodFrameIndex = stackTrace.GetFrames().Select((f, i) => new { i, assertQuery = assertQueryMethods.Any(aqm => f.GetMethod().Name.StartsWith(aqm)) }).Where(r => r.assertQuery).LastOrDefault().i;
                    ...
                    //else if (exception.Message.Contains(@"A column has been specified more than once in the order by list. Columns in the order by list must be unique."))
                    //{
                    //}
                    else
                    {
